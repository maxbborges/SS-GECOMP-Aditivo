...
function loadClicks() {
    $('#btn_CarregarProceso_00').on("click", function () {
        ...
        if (nprocessoAnterior == null || nprocessoAnterior == '') { loading.hide(); msgsToast("Necessário digitar algum processo para buscar!", 'danger') }
        else {
            ...
            if (datasetSolicitacoes.length == 0) { loading.hide(); msgsToast("Processo não encontrado!", 'danger'); }
    ...
    $('#btn_ass_notaTecnica').on("click", function () {
        customAssinatura($('[name="txt_nrPasta"]').val(), $('[name="txt_nomeArquivo_27"]').val(),
            $('[name="txt_idDoc27"]').val(), nProcesso, ['3', '2'], '3', [], [], [], '[name="txt_03_validaVertsign"]')
    });

    $('#btn_ass_novaNotaTecnica').on("click", function () {
        customAssinatura($('[name="txt_nrPasta"]').val(), $('[name="txt_nomeArquivo_07"]').val(),
            $('[name="txt_idDoc07"]').val(), nProcesso, ['7', '200'], '200', [], [], [], '[name="txt_28_validaVertsign"]')
    });

    $('#btn_ass_parecerJuridico').on("click", function () {
        customAssinatura($('[name="txt_nrPasta"]').val(), $('[name="txt_nomeArquivoParecer_10"]').val(),
            $('[name="txt_idDoc10Parecer"]').val(), nProcesso, ['172', '14', '16'], '16', [], [], [], '[name="txt_14_validaVertsign"]')
    });
    ...
    $('#btn_ass_minuta').on("click", function () {
        ...
        customAssinatura($('[name="txt_nrPasta"]').val(), $('[name="txt_nomeArquivoMinuta_10"]').val(),
            $('[name="txt_idDoc10Minuta"]').val(), nProcesso, ['15', '14', '16'], '43', tabela, [], [], '[name="txt_15_validaVertsign"]')
    });
}
...
function consultaContrato(){
    ...
    let c_filial = $('[name="txt_codFilialAnterior"]').val()
    console.log(c_chave)
    console.log(c_filial)
    if (c_filial == null || c_chave == null) {msgsToast("Valores errados!")}
    constraints = [
    ...
    if (datasetContrato.length == 0) { loading.hide(); msgsToast('Erro no Protheus!', 'danger'); }
    return datasetContrato[datasetContrato.length - 1]
}

function loadDatasets(c_filial, sc) {
    let c_chave = $('[name="txt_nContratoAnterior"]').val()
    var contrato = consultaContrato()
    ...
    constraints = [
        DatasetFactory.createConstraint('numeroContrato', numeroContrato, '', ConstraintType.MUST),
        DatasetFactory.createConstraint('filial', c_filial, c_filial, ConstraintType.MUST),
    ]
    ...
    if (c_filial=='010001'){
        $("[name='rd_entidade_02']:first").prop( "checked", true );
        $("[name='rd_entidade_02']:not(:first)").prop('disabled', 'true');
    } else {
        $("[name='rd_entidade_02']:not(:first)").prop( "checked", true );
        $("[name='rd_entidade_02']:first").prop('disabled', 'true');
    }
    ...
    $('[name="txt_valor_inicial"]').maskMoney({thousands:'.', decimal:',', prefix: 'R$ '}).maskMoney('mask', datasetAdicionais[0].CN9_VLINI)
    $('[name="txt_valor_atual"]').maskMoney({thousands:'.', decimal:',', prefix: 'R$ '}).maskMoney('mask', datasetAdicionais[0].CN9_VLATU)
    // $('[name="txt_valor_inicial"]').val(datasetAdicionais[0].CN9_VLINI)
    // $('[name="txt_valor_atual"]').val(datasetAdicionais[0].CN9_VLATU)

    // $('[name="_txt_vigencia_19"]').val(contrato.CN9_VIGE) // 001
    // $('[name="_txt_novaVigencia_19"]').val(contrato.CN9_UNVIGE) // VERIFICAR NOVO VALOR // 001
    if (datasetRevisao.length > 0) { $('[name="txt_revisao_19"]').val(datasetRevisao[0].CN9_REVISA) } // 001
    else { $('[name="txt_revisao_19"]').val('000') }// 001
    if (datasetAdicionais[0].CN9_FLGREJ == '1') { txtReajuste = 'Sim' }
    else { txtReajuste = 'Não' } // 007
    $('[name="txt_reajuste_21"]').val(txtReajuste) // 007
    $('[name="slc_gpAtual_23"]').val(listaDeGpAprovacao[contrato.CN9_APROV]) // 010
    $('[name="txt_codAnterior_24"]').val(fornecedores[0].CNA_FORNEC) // 012
    $('[name="txt_lojaAnterior_24"]').val(fornecedores[0].CNA_LJFORN) // 012
    $('[name="txt_CNPJAnterior_24"]').val(fornecedores[0].A2_CGC) // 012
    $('[name="txt_RazaoAnterior_24"]').val(fornecedores[0].A2_NOME) // 012
...
function loadCollapse() {
    ...
    else if (ATIVIDADE == ACTIVITY.CADASTRAR_NO_PROTHEUS) { hideBlockDivs(['div_17', 'div_18', 
        'div_30','div_19','div_20','div_21','div_22','div_23','div_24']) }// DIV_17, DIV_18, DIV_30
    ...
    else if (ATIVIDADE == ACTIVITY.APROVAR_GESTOR_SOLICITANTE) {hideBlockDivs(['div_25'])}
...
function loadZooms() {
    ...
    createFilterZoom('ds_consulta_fornecedor', [], [], [], 'A2_CGC', false, 'Escolha a filial', header, ['A2_LOJA', 'A2_NOME','A2_CGC','A2_COD'], '#zf_fornecedores_24')

    header = [
        { 'title': 'CN6_CODIGO', 'size': 'col-xs-3' },
        { 'title': 'CN6_DESCRI', 'size': 'col-xs-9', 'standard': true }
    ]
    createFilterZoom('ds_consulta_indice', [], [], [], 'CN6_DESCRI', false, 'Escolha o índice', header, ['CN6_CODIGO', 'CN6_DESCRI'], '[name="zf_indice_21"]')
    ...
    createFilterZoom('ds_consulta_grupo_aprovacao', [], [], [], 'AL_DESC', false, 'Escolha o grupo', header, ['AL_DESC', 'AL_USER', 'AL_COD'], '[name="zf_gpAtual_23"]')
}
...
var beforeSendValidate = function (numState, nextState) {
    if (numState == ACTIVITY.INICIO || numState == ACTIVITY.ZERO) { validaCamposRequired('div_00') }
        ...
        validaCamposRequired('div_01'); validaCamposRequired('div_02'); validaCamposRequired('div_27')
        if ($("input[name^='column1_1___']").length < 1) { ExibirMensagem.msg("Necessário incluir ao menos um registro de alteração!") }
    }
    else if (numState == ACTIVITY.APROVAR_SOLICITACAO) { validaCamposRequired('div_03') }
    ...
    else if (numState == ACTIVITY.APROVACAO_GERENCIA) { validaCamposRequired('div_28') }
    ...
    else if (numState == ACTIVITY.APROVAR_ASSESSOR) { validaCamposRequired('div_14') }
    ...
    else if (numState == ACTIVITY.APROVAR_GESTOR_SOLICITANTE) {validaCamposRequired('div_25');}
}
